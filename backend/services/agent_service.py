...
def _extract_execution_result_from_summary(summary: str) -> dict[str, Any] | None:
    raw = (summary or "").strip()
    if not raw or not raw.startswith("{"):
        return None
    try:
        ...
        return None
    result = payload.get("execution_result") if isinstance(payload, dict) else None
...
def _serialize_execution_result(execution_result: Any) -> str:
    """Persist structured results in the existing jobs.summary text column."""
    if isinstance(execution_result, ExecutionResult):
        payload = execution_result.to_dict()
        payload.pop("workspace_path", None)
        return json.dumps(
            {"message": execution_result.summary, "execution_result": payload},
            ensure_ascii=False,
        )
    return str(execution_result or "")
...
async def _apply_clarification_answer_to_spec(job_id: str, *, conversation_id: str) -> None:
    ...
    if updated:
        persistence_result = _db_update(job_id, {"specification": spec_dict})
        if inspect.isawaitable(persistence_result):
            await persistence_result
        logger.info(
...
def _row_to_response(row: dict[str, Any]) -> JobResponse:
    return JobResponse(
        ...
        execution_result=_extract_execution_result_from_summary(str(row.get("summary") or "")),
...
async def run_agent_pipeline(*, job_id: str, payload: JobCreate, user_id: str, trace_id: str | None = None) -> None:
    ...
    try:
        ...
        if result.data:
            ...
            # Both states can reach this path: the answer handler transitions
            # WAITING_FOR_USER to RESUMED before redispatching the job.
            if current_status in {
                JobStatus.WAITING_FOR_USER.value,
                JobStatus.RESUMED.value,
            }:
                logger.info("[resume] job %s resuming from %s", job_id, current_status)
    ...
    if intent == "code":
        ...
        _db_update(
            job_id,
            {"status": final_status, "summary": _serialize_execution_result(execution_result)},
        )
        if payload.workspace_id:
            try:
    ...
    if intent == "chat":
        ...
        _db_update(
            job_id,
            {
                ...
                "summary": _serialize_execution_result(execution_result),
            },
        )
        obs.emit(
...
def to_forge_v2_response(job: JobResponse) -> dict[str, Any]:
    ...
    structured_result = dict(job.execution_result or {})
    public_result = dict(structured_result)
    public_result.pop("workspace_path", None)
    deployment = public_result.get("deployment") or _extract_deployment_from_summary(raw_summary)
    ...
    if public_result:
        run["execution_result"] = public_result
    error: str | None = None
