...
class DeployService:
    ...
    @staticmethod
    def _require_workspace_domain(domain: str, base_domain: str) -> str:
        ...
        if (
            ...
            or not valid_labels
            or cleaned == base
            or not cleaned.endswith(f".{base}")
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "code": "invalid_workspace_domain",
                    "message": "Workspace domain must be a valid subdomain of the configured base domain",
                },
            )
        return cleaned
        ...
        *,
        ...
        if allow_write is not True:
            return {
                "success": False,
                "stdout": "",
                "stderr": "Deployment write permission was not granted",
                "exit_code": 13,
                "url": None,
                "reused_domain": False,
            }
        ...
        server_name = DeployService._require_workspace_domain(
            cleaned_domain or f"{slug}.{base_domain}",
            base_domain,
        )
