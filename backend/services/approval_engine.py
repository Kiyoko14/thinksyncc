...
def _reliability():
    """Lazy import of the shared reliability layer.

    Avoids a circular import between approval_engine and conversation_reliability
    (which imports ApprovalEngine to call ``_persist`` from ``save_approval_atomic``).
    """
    from services.conversation_reliability import (
        IdempotencyError,
        IdempotencyGuard,
        OptimisticLockError,
        OptimisticLockGuard,
    )
    return IdempotencyError, IdempotencyGuard, OptimisticLockError, OptimisticLockGuard


def _token_revocation_engine():
    """Lazy import of the single token-revocation engine.

    Avoids importing interactive_wait at module load (which itself imports
    approval-related types)."""
    from services.interactive_wait import _TokenRevocationEngine
    return _TokenRevocationEngine
...
class ApprovalEngine:
    ...
    async def resolve(
        ...
        *,
        ...
        IdempotencyError, IdempotencyGuard, OptimisticLockError, OptimisticLockGuard = _reliability()
        ...
        if decision == ApprovalDecision.REJECTED:
            await _token_revocation_engine().revoke_for_approval(
    ...
    async def _persist(self, request: ApprovalRequest) -> None:
        ...
        try:
            _, _, OptimisticLockError, _ = _reliability()
            if is_create:
