...
async def _run_startup_diagnostics() -> dict[str, Any]:
    ...
    # Fail fast if the durable-queue tables are missing. The durable job queue
    # is a runtime hard dependency; without these tables workers cannot claim,
    # heartbeat, or record execution, and job state is silently lost.
    if missing_tables:
        raise RuntimeError(
            "Startup aborted — required database tables are missing: "
            f"{', '.join(missing_tables)}. Apply pending migrations before starting."
        )
...
# Paths whose HTTP bodies contain bearer tokens or credentials and must never
# be logged in full. Only method/status metadata are recorded for these paths.
_SECRET_PATH_PREFIXES: tuple[str, ...] = (
    "/auth/",
    "/servers",  # ssh_key/ssh_password on POST/GET
)


def _is_secret_path(path: str) -> bool:
    p = (path or "").lower()
    return any(p.startswith(prefix) for prefix in _SECRET_PATH_PREFIXES)
...
async def log_http_requests(request: Request, call_next):  # type: ignore[override]
    ...
    # Only parse and log a bounded JSON body for non-secret paths. For
    # secret-carrying paths log only metadata to avoid capturing tokens or
    # credentials. Never log the Authorization header verbatim.
    if not secret_path:
        ...
        logger.info(
            "[http] request | method=%s | path=%s | query=%s | body=<redacted>",
            request.method,
            path,
            request.url.query,
        )
    ...
    try:
        ...
        logger.exception("[http] exception | method=%s | path=%s", request.method, path)
        raise

    # Do NOT reconstruct the response body — that breaks streaming and
    # background tasks and forces every response through memory. Log
    # only the status code. Body-level logging is a debug-only concern.
    logger.info(
...
async def handle_postgrest_error(request: Request, exc: APIError) -> JSONResponse:
    ...
    if code == "23503":
        # Foreign-key violation → resource does not exist / relationship missing.
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,
            content=_error_payload(message or "Referenced resource missing.", request=request, code=code, exc=exc),
        )
    if code == "23505":
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,
            content=_error_payload(message or "Duplicate resource.", request=request, code=code, exc=exc),
        )
    return JSONResponse(
