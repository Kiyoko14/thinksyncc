...
from services.agent_service import (
    _extract_execution_result_from_summary,
    _serialize_execution_result,
    to_forge_v2_response,
)
...
def test_structured_execution_result_round_trips_without_workspace_path() -> None:
    projected = ExecutionResultProjector.project(
        {
            "success": True,
            "workspace_path": "/root/workspaces/ws-1",
            "created_files": ["index.html"],
            "deployment": {"url": "https://taskflow.example.com", "verified": True},
        },
        workspace={"id": "ws-1", "display_name": "TaskFlow", "slug": "taskflow"},
    )
    stored = _serialize_execution_result(projected)
    result = _extract_execution_result_from_summary(stored)

    assert result is not None
    assert result["created_files"] == ["index.html"]
    ...
    response = to_forge_v2_response(job)
    public_result = response["run"]["execution_result"]
    assert public_result["deployment"]["verified"] is True
    assert "workspace_path" not in public_result
