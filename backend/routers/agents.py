...
async def forge_v2_plan(
    ...
    """Plan-only endpoint (currently not implemented as a separate path).

    The unified execution pipeline generates and executes the plan atomically
    inside a queued job. A dedicated plan-only endpoint is intentionally
    unavailable to prevent divergent plan/execute code paths.
    """
    logger.info(
        "[agents] forge_v2_plan called (unsupported) | user_id=%s",
        current_user.get("sub"),
    )
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail={
            "code": "PLAN_ONLY_NOT_SUPPORTED",
            "message": (
                "Plan-only execution is not supported. Submit the job via "
                "POST /agents/forge-v2/run and inspect the generated plan "
                "via GET /agents/forge-v2/jobs/{job_id}."
            ),
        },
    )
