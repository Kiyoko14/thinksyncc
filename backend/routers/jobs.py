...
async def get_recovery_report(
    ...
    """Return a recovery report scoped to the authenticated user's jobs.
    ...
    def _filter_bucket(bucket: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(bucket, dict):
            return bucket
        rows = [j for j in (bucket.get("jobs") or []) if j.get("user_id") == user_id]
        return {"count": len(rows), "jobs": rows}

    for key in ("unfinished", "orphaned", "orphaned_db_only", "recoverable"):
        if key in report:
            report[key] = _filter_bucket(report[key])
    return report
