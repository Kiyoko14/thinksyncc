...
export interface DeploymentResponse {
  ...
  slug: string;
  is_active: boolean;
  runtime: "static" | "python" | "node" | string;
  verified: boolean;
}

export const getWorkspaceDeployment = (workspace_id: string) =>
  request<DeploymentResponse>(`/api/deployments/${workspace_id}`);

export const createWorkspaceDeployment = (workspace_id: string) =>
  request<DeploymentResponse>(`/api/deployments/${workspace_id}`, { method: "POST" });

export const deactivateWorkspaceDeployment = (workspace_id: string) =>
  request<{ message: string }>(`/api/deployments/${workspace_id}`, { method: "DELETE" });
...
export interface ExecutionResultDeployment {
  url?: string | null;
...
export interface ForgeV2RunResponse {
  ...
  summary: string;
  success: boolean;
  execution_result?: StructuredExecutionResult | null;
  deployment?: ExecutionResultDeployment | null;
}
