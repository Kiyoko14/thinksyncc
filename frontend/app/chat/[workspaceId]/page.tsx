...
import {
  ...
  type JobStreamEvent,
  type StepResult,
  type ExecutionResultDeployment,
  type StoredChatMessage,
  type ChatRole,
  type Workspace,
  type ClarificationForm as ClarificationFormType,
  type ClarificationQuestion,
  type ClarificationFormSubmission,
} from '@/services/api';
...
interface ChatMessage {
  ...
  steps?: StepResult[];
  deployment?: ExecutionResultDeployment | null;
  clarificationForm?: ClarificationFormType | null;
...
export default function ChatPage() {
    ...
    setMessages(prev => prev.map(msg => 
      msg.id === messageId 
        ? {
            ...
            deployment: job.run?.execution_result?.deployment || job.run?.deployment || msg.deployment,
          }
        : msg
    ));
  ...
  const connectToJob = useCallback((jobId: string, messageId: string) => {
    ...
    socketRef.current.onmessage = (event) => {
      try {
        ...
        if (data.type === 'waiting_for_clarification') {
          setMessages(prev => prev.map(msg =>
            msg.id === messageId
              ? {
                  ...msg,
                  status: 'waiting_for_user',
                  clarificationForm: data.clarification_form,
                  clarificationQuestions: data.questions,
                }
              : msg
          ));
        } else if (data.type === 'completed' || data.type === 'step_result') {
  ...
  const handleClarificationSubmit = async (
    messageId: string,
    jobId: string,
    submission?: ClarificationFormSubmission | null,
    textReply?: string | null
  ) => {
    try {
      await submitClarificationReply(jobId, {
        clarification_submission: submission,
        reply: textReply,
      });
      setMessages(prev => prev.map(m =>
        m.id === messageId
          ? { ...m, status: 'running', clarificationForm: null, clarificationQuestions: null }
          : m
      ));
      pollJobStatus(jobId, messageId);
    } catch (err) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, jobId: job.job_id } : m));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
      <WorkspaceHeader workspace={workspace} router={router} />
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/40">
            <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="inline-flex items-center rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                  {workspace?.display_name || workspace?.name || 'Loading workspace'}
              </span>
            </div>
...
const WorkspaceHeader = ({ workspace, router }: { workspace: Workspace | null, router: any }) => (
  <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-3">
      ...
      <h1 className="text-xl font-semibold text-gray-900">{workspace?.display_name || workspace?.name || 'Chat'}</h1>
    </div>
...
const ChatMessageItem = ({
  ...
  const handleFormSubmit = async (submission: ClarificationFormSubmission) => {
    if (!msg.jobId) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      await onClarificationSubmit(msg.id, msg.jobId, submission, null);
    } catch (err) {
  ...
  const handleTextSubmit = async (reply: string) => {
    if (!msg.jobId) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      await onClarificationSubmit(msg.id, msg.jobId, null, reply);
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'Submission failed');
    } finally {
      setSubmitting(false);
    ...
    <div className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
      ...
      <div className={`max-w-2xl ${msg.role === 'user' ? 'order-first' : ''}`}>
        <div className={`rounded-2xl px-4 py-3 shadow-sm border ${
          ...
          {msg.deployment && (
            <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
              <p className="font-semibold">
                {msg.deployment.verified ? 'Deployment Verified' : 'Deployment configured; verification pending or failed'}
              </p>
              {msg.deployment.url && (
                <a className="mt-1 inline-block font-medium underline" href={msg.deployment.url} target="_blank" rel="noreferrer">
                  Open deployed app
                </a>
              )}
              {msg.deployment.error && <p className="mt-1 text-xs text-emerald-800">{msg.deployment.error}</p>}
            </div>
          )}
          {msg.status && msg.status !== 'completed' && msg.status !== 'waiting_for_user' && (
            <div className="flex items-center gap-2 mt-3 text-xs text-gray-500">
              <Loader2 className="animate-spin" size={14}/>
              <span>{msg.status.charAt(0).toUpperCase() + msg.status.slice(1)}...</span>
            </div>
          )}
          {msg.status === 'waiting_for_user' && msg.jobId && msg.clarificationForm && (
            <div className="mt-3">
              <ClarificationForm
          ...
          {msg.status === 'waiting_for_user' && msg.jobId && !msg.clarificationForm && msg.clarificationQuestions && (
            <div className="mt-3">
              <WaitingCard
                kind="clarification"
